<?php
namespace PlugHacker\PlugPagamentos\Model\ResourceModel\Invoices;
...
class Collection extends SearchResult
{
    ...
    protected function _renderFiltersBefore()
    {
        ...
        $joinTable = $this->getTable('plug_module_core_transaction');

        $this->getSelect()
            ->joinLeft(
                $joinTable,
                "main_table.plug_id = {$joinTable}.charge_id ",
                [
                    'id' => new \Zend_Db_Expr('GROUP_CONCAT(main_table.id)'),
                    'tran_id' => new \Zend_Db_Expr("GROUP_CONCAT({$joinTable}.id)"),
                    'tran_plug_id' => new \Zend_Db_Expr("GROUP_CONCAT({$joinTable}.plug_id)"),
